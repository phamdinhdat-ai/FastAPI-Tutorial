...
from .basemodels import XrayModel
...
class Predictor:
    ...
    def load_model(self):
        try:
            model = XrayModel(XrayDataConfig.N_CLASSES)
    ...
    def create_transform(self):
        self.transforms_ = torchvision.transforms.Compose([
            transforms.Resize((XrayDataConfig.IMG_SIZE, XrayDataConfig.IMG_SIZE)),
            transforms.CenterCrop(XrayDataConfig.IMG_SIZE),
            transforms.ToTensor(),
            transforms.Normalize(mean = XrayDataConfig.NORMALIZE_MEAN, std = XrayDataConfig.NORMALIZE_STD)
        ])
    ...
    def output2pred(self, output):
        ...
        predicted_class = XrayDataConfig.ID2DLABEL[predicted_id]
